 ...
 import "./Bill.css";
   ...
   function Bill ({data,onBack, onPaymentSuccess,onUpdateCart,onRemoveItem})
   {
    ...
    useEffect(() =>{
        ...
        if(data.cid)
        {
            axios.get(`${process.env.REACT_APP_BACKEND_URL}/customer/getcustomerdetails/${data.cid}`).then((res) => {
                const body = res.data || {};
                setCustomer({
    ...
    const saveBill = useCallback(async () => {
         ...
         const res = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/bill/getbillid`);
         ...
         for (const item of items)
         {
            ...
            await axios.post (`${process.env.REACT_APP_BACKEND_URL}/bill/billsave`, {
                billid : nextId,
                billdate: today,
                cid: data.cid,
                pid : item.pid,
            ...
            await axios.post(`${process.env.REACT_APP_BACKEND_URL}/sales/add`,{
    ...
    const displayRazorpay = async () => {
        ...
        try{
            ...
            const order = await axios.post(`${process.env.REACT_APP_BACKEND_URL}/payment/orders/${amountInPaisa}`);
        ...
        paylater: true
                ...
                handler: async function (response)
                {
                    ...
                    try {
                        await axios.post(`${process.env.REACT_APP_BACKEND_URL}/paymentdetails/paymentdetailsave`,{
...
//    export default Bill;
   ...
   //                 try
    ...
    //                         orderCreationId:order_id,
    //                         rozorpayPaymentId:response.rozorpay_payment_id,
    //                         rozorpayOrderId: response.rozorpay_order_id,
    //                         rozorpaySignature: response.rozorpay_signature,
    //                         cid: data.cid,
    //                         amount:amount / 100,
    //                         billid:saveBillId,
    //                     });
    ...
    //                     if(!invRes.success)
    //                     {
    //                         alert("Payment successed, but inventory update failed. Cheack server logs.");
    //                     }
    //                     else
    //                     {
    //                         setIsPaymentDone(true);
    //                         alert("Payment Successfully and inventory updated");
    //                     }
    //                     onPaymentSuccess?.();
    //                 }
    //                 catch(err)
    //                 {
    //                     alert("Payment successed but post.proccessing failed. check console.");
    //                     onPaymentSuccess?.();
    //                 }
    ...
    //             notes : {address:"universal Informatics Indore Pvt.Ltd."},
    //             theme: {color: "#fb a161ff"},
    //         };
    //         new window.Rozorpay(options).open();
    //     }
    //     catch(err)
    //     {
    //         alert("Could not create payment order. Cheack server");
    //         setIsProcessing(false);
    //     }
    // };
